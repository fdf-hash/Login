<template>
  <div class="forget-pass">
    <div class="fy_content">
      ...
      <div class="fy_box">
        <div class="fy_nums">
          <input type="text" maxlength="11" placeholder="请输入手机号" v-model="sun_shouji" />
          <span @click="huo()">获取验证码</span>
        </div>
        <div class="fy_num">
          <input type="text" maxlength="11" placeholder="请输入验证码" v-model="sun_dx" />
        </div>
        <div class="fy_num">
          <input type="text" maxlength="11" placeholder="请输入密码" v-model="sun_pass" />
        </div>
        <!-- 点击确定按钮 -->
        <div class="fy_login">
          <van-button round type="warning" eb6100 size="large" class="fy_button" @click="fy_button()">确定</van-button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    ...
    mounted(){
            this.sun_shouji = this.$route.params.mobile
    },
  methods: {
    ...
    fy_button() {
      if (this.sun_shouji == "") {
        ...
        alert("请先获取验证码");
        return false;
      } else if (this.sun_pass == "") {
        alert("密码不能为空");
        return false;
      }else{
          this.axios.post("https://test.365msmk.com/api/app/password",{
                mobile:this.sun_shouji,  //手机号 
                password:this.sun_pass,	    //输入的密码
                sms_code: this.sun_dx        //收到的验证码
          }).then((res)=>{
